...
The environment part tells GitHub this job is deploying to the pages environment and grabs the live url once its done. It also ties into them permissions above it, since Pages deployment need that specific access to work.

Auto deployment is more rdeliable cause it does the same steps the same way every single time, it dont forget to check links or skip anythung by accident. It also stops broken code from going live since deploy won't run unless build/test passes first.
